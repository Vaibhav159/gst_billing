...
class LineItemAPITestCase(BaseAPITestCase):
    ...
    def test_partial_update_line_item(self):
        ...
        url = reverse("lineitem-detail", args=[self.line_item.id])
        data = {"product_name": "Partially Updated Product"}
        response = self.client.patch(url, data, format="json")

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.line_item.refresh_from_db()
        self.assertEqual(self.line_item.product_name, "Partially Updated Product")
    ...
    def test_filter_line_items_by_invoice(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(response.data["results"][0]["product_name"], "Another Product")
    ...
    def test_create_line_item_for_invoice(self):
        ...
        self.assertEqual(new_line_item.product_name, "API Created Product")
